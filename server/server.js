...
app.post('/api/clients', authenticateToken, upload.array('photos'), async (req, res) => {
  const { clientName, clientEmail, clientPassword } = req.body; // Adicionado clientEmail
  const files = req.files;
  
  if (!clientName || !clientEmail || !clientPassword || !files || files.length === 0) { // Adicionada verificação para clientEmail
  ...
  try {
    ...
    const clientUser = await admin.auth().createUser({
      email: clientEmail, // Usar o e-mail fornecido no formulário
    ...
    await firestore.collection('clients').doc(clientId).set({
      name: clientName,
      email: clientEmail, // Opcional: salvar o e-mail no Firestore também
      photos: photoUrls,
